...
export async function updateAttention(
  ...
  try {
    db.transaction((tx) => {
      // Devolver el stock de los productos que tenía la atención
      const oldProducts = tx
        .select()
      ...
      tx.delete(attentionServices).where(eq(attentionServices.attentionId, id)).run();
      tx.delete(attentionProducts).where(eq(attentionProducts.attentionId, id)).run();
      ...
      tx.update(attentions)
        .set({ petName, ownerName, date, notes, discount, total })
